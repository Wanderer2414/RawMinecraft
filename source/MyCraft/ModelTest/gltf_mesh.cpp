...
GLTFStaticMesh::GLTFStaticMesh(const char* filename, float scale){
    ...
    if(!error.empty()) {
        std::cerr << "Error loading GLTF file: " << error << std::endl;
    }
    if(!warning.empty()) {
        std::cout << "Warning loading GLTF file: " << warning << std::endl;
    }
    this->scale = scale;
...
std::pair<unsigned int, std::map<int, unsigned int>> GLTFStaticMesh::bindModel(){
    ...
    for(auto it = ebos.cbegin(); it != ebos.cend();){
        ...
        if(bufferView.target != GL_ELEMENT_ARRAY_BUFFER){
            // Remove non-element array buffers
...
void GLTFStaticMesh::bindMesh(std::map<int, unsigned int>& ebos, tinygltf::Mesh & mesh){
    ...
    for(int i = 0; i < mesh.primitives.size(); ++i){
        ...
        for(auto & attrib : primitive.attributes){
            ...
            else if(attrib.first.compare("TEXCOORD_0") == 0) attribute = 1;
            else if(attrib.first.compare("NORMAL") == 0) attribute = 2;
            if(attribute > 0){
