...
namespace MyCraft {
    CraftingTableUI::CraftingTableUI(ItemTable* table): __items(*table),__currentRecipe(0), __output(0), __hoverItem(0) {
        memset(__recipe, 0, sizeof(void*)*9);
        MyBase::Texture texture("assets/images/CraftingTable.png");
    ...
    glm::vec2 CraftingTableUI::getSize() const {
        return __inventoryTexture.getSize();
    };
    glm::vec2 CraftingTableUI::getPosition() const {
        return __inventoryTexture.getPosition();
    };

    bool CraftingTableUI::setHover(const glm::vec2& position) {
        ...
        if (offset.x>=509 && offset.x<=1601) {
            ...
            if (offset.x<=102) {
                if (offset.y>=9 && offset.y<=111) {
                    ...
                    if (offset.y>=539 && offset.y <= 861) {
                        if (n>=2 && n<=4) {
                            offset.y -= 539;
                            int y = floor(offset.y/110);
                            offset.y -= y*110;
                            if (offset.y<=102) {
                                __hoverPosition = {-1, (2-y)*3 + (n-2)};
                                __chosenTexture.setVisible(true);
                                __chosenTexture.setTextureExportPosition(getRecipiPosition(glm::ivec2(2-y, n-2)));
                                return true;
                            }
                        }
                        else if (n==7 && offset.y>=649.f && offset.y<=751) {
                            __hoverPosition = {-2, 0};
                            __chosenTexture.setVisible(true);
    ...
    void CraftingTableUI::update() {}
    
    void CraftingTableUI::close() {
        for (int i = 0; i<9; i++) {
            if (__recipe[i]) {
    ...
    void CraftingTableUI::open() {
        for (glm::ivec2 position(0,0); position.x<4; position.x++) {
            for (position.y = 0; position.y<10; position.y++) 
                if (getBags(position))
                    getBags(position)->setPosition(getItemPosition(position) + __items.package.size*0.125f);
        }
    }
    
    bool CraftingTableUI::catchEvent(GLFWwindow* window) {
    ...
    bool CraftingTableUI::__mouseClicked(GLFWwindow* window) {
            ...
            if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT)) {
                if (!__hoverItem && getBags(__hoverPosition)) {
                    if (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT)) {
                        ...
                        if (same != -1) {;
                            item = getBags({-1, same})->merge(item);
                            placeBags(__hoverPosition, item);
                            __reCheckRecipe();
                        }
                        else if (empty != -1) {
                            placeBags({-1, empty}, item);
                            item->setPosition(getItemPosition({-1, empty}) + __items.package.size*0.125f);
                        }
                    }
                    else {
                ...
                else if (__hoverPosition.x == -2 && __output) {
    ...
    bool CraftingTableUI::__mouseDown(GLFWwindow* window) {
    ...
    bool CraftingTableUI::contains(const glm::vec2& position) const {
    ...
    void CraftingTableUI::glDraw() const {
        MyBase::Container2D::glDraw();
        for (int i = 0; i<4; i++) {
            for (int j = 0; j <10; j++)
                if (getBags({i,j})) getBags({i,j})->draw();
        }
        for (int i = 0; i<9; i++)
    ...
    Item* CraftingTableUI::getBags(const glm::ivec2& position) const {
        if (position.x>=0) return __items.getBags(position);
        else if (position.x == -1) return __recipe[position.y];
        return 0;
    }

    Item* CraftingTableUI::placeBags(const glm::ivec2& position, Item* item) {
    ...
    void CraftingTableUI::__reCheckRecipe() {
        if (__currentRecipe) {
    ...
    glm::vec2 CraftingTableUI::getToolbarPosition(const int& n) const {
        return glm::vec2((509.f + n*110.f)/1610, 9.f/940)*getSize() + getPosition();
    }

    glm::vec2 CraftingTableUI::getStoragePosition(const glm::ivec2& offset) const {
        return glm::vec2((509.f + offset.y*110.f)/1610, (139.f+110.f*offset.x)/940)*getSize() + getPosition();
    }
    glm::vec2 CraftingTableUI::getRecipiPosition(const glm::ivec2& offset) const {
        return glm::vec2((729.f + offset.y*110.f)/1610, (759.f-110.f*offset.x)/940)*getSize() + getPosition();
    }
    glm::vec2 CraftingTableUI::getOutputItemPosition() const {
        return glm::vec2(1279.f/1610, 649.f/940)*getSize() + getPosition();
    }
    glm::vec2 CraftingTableUI::getItemPosition(const glm::ivec2& position) const {
        if (position.x == 3) return getToolbarPosition(position.y);
        if (position.x >= 0) return getStoragePosition(position);
        if (position.x == -1) return getRecipiPosition({position.y/3, position.y%3});
        if (position.x == -2) return getOutputItemPosition();
        return {0,0};
    }
}