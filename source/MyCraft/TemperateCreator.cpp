...
namespace MyCraft {
        ...
        for (glm::ivec2 cPosition(biome.getPosition().x, 0); cPosition.x < maxX; cPosition.x++) {
            for (cPosition.y=biome.getPosition().y; cPosition.y<maxY; cPosition.y++) {
                ...
                switch (biome[cPosition].type) {
                    case Biome::SuperLow: biome[cPosition].type = Biome::Beach; break;
                    case Biome::Low: biome[cPosition].type = Biome::Meadow; break;
                    case Biome::Mid: biome[cPosition].type = Biome::Meadow; break;
                    case Biome::High: biome[cPosition].type = Biome::MixRockyHill; break;
                    case Biome::SuperHigh: biome[cPosition].type = Biome::FrozenPeak; break;
                    default: break;
                }
            }
        }
    ...
    void Temperate::FillColumn(Chunk& chunk, HeightMap& map, Biome& biome, const glm::ivec3& ceiling) const {
        switch (biome.type) {
            case Biome::Meadow: {
            ...
            case Biome::FrozenPeak: {
                int mod = ceiling.z%16;
                if (mod<0) mod = 16 - (-ceiling.z)%16;
                int rate = std::min(rand()%5 + 3, mod);
                for (glm::ivec3 position(ceiling.x,ceiling.y, ceiling.z-rate); position.z<=ceiling.z; position.z++)
                    chunk.setType(position, Ice);
            }
                break;
    ...
    void Temperate::FillChunk(const std::string& src, MyBase::Image& image, HeightMap& map, const glm::ivec3& origin, const Biome& biome) const {
        switch (biome.type) {
            case Biome::Beach:{
            ...
            case Biome::Meadow: {
                ...
                break;
            case Biome::FrozenPeak: {
                image.setColor(origin.x, origin.y, WHITE); 
            }
                break;
            case Biome::Sea:{
            }
    ...
    void Temperate::createOakTree(DynamicChunk& chunk, const glm::ivec3& root) const {
        int height = rand()%3+5;
